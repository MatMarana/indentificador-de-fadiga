...
def detectar_ponto_fadiga(
    ...
    "derivada" → argmax do gradiente (transição abrupta)
    "limiar"   → primeiro cruzamento do percentil_limiar (progressão gradual)
    """
    if metodo == "derivada":
...
def pinn_loss(
    ...
    L_data — MSE contra o score fisiológico (target real de fadiga)
    L_mono — penaliza df/dt < 0 via ReLU(-grad): fadiga é crescente
    L_reg  — penaliza d²f/dt² (suavidade de Tikhonov)
    """
    x      = x.clone().requires_grad_(True)
    y_pred = modelo(x)

    # Gradiente em relação a todas as features
    ...
    # Norma do gradiente por amostra → escalar de "taxa de variação"
    grad_norm = grads_1.norm(dim=1, keepdim=True) # (N, 1)

    # Gradiente de segunda ordem da norma
    grads_2 = torch.autograd.grad(
        outputs=grad_norm,
        inputs=x,
        grad_outputs=torch.ones_like(grad_norm),
        create_graph=True,
        retain_graph=True,
    )[0]
    grad2_norm = grads_2.norm(dim=1, keepdim=True)

    L_data  = nn.MSELoss()(y_pred, y)
    L_mono  = torch.mean(torch.relu(-grad_norm))   # monotonicidade
    L_reg   = torch.mean(grad2_norm ** 2)          # suavidade
...
def comparar_pinn_vs_fisiologico(
    ...
    idx_fisio, t_fisio = detectar_ponto_fadiga(fadiga_suave, tempo, metodo="derivada")
    idx_pinn,  t_pinn  = detectar_ponto_fadiga(pred,         tempo, metodo="derivada")
...
def main():
    ...
    for nome_arq in ARQUIVOS_TESTE:
        ...
        idx_fisio, tempo_fadiga_fisio = detectar_ponto_fadiga(
            fadiga_suave_t, t_janelas_t, metodo="derivada"
        )
        ...
        idx_pinn, tempo_fadiga_pinn = detectar_ponto_fadiga(
            pred_t, t_janelas_t[:n_t], metodo="derivada"
        )

        # Comparação PINN vs Fisiológico
        comparar_pinn_vs_fisiologico(
            fadiga_suave_t[:n_t], pred_t, t_janelas_t[:n_t], nome_arq
        )
